    ...
    try:
        with open(path, "r") as blob:
            ...
            if data["schemaVersion"] == 1:
                result = set([entry["blobSum"].split(":")[1]
                              for entry in data["fsLayers"]])
            else:
                result = set([entry["digest"].split(":")[1]
...
class RegistryCleaner(object):
    ...
    def _delete_layer(self, repo, digest):
        ...
        path = os.path.join(self.registry_data_dir,
                            "repositories", repo, "_layers/sha256", digest)
        self._delete_dir(path)

    def _delete_blob(self, digest):
        """remove blob directory from filesystem"""
        path = os.path.join(self.registry_data_dir,
                            "blobs/sha256", digest[0:2], digest)
        self._delete_dir(path)
    ...
    def _get_tags(self, repo):
        ...
        path = os.path.join(self.registry_data_dir,
                            "repositories", repo, "_manifests/tags")
        ...
        for other_tag in [t for t in self._get_tags(repo) if t != tag]:
            ...
            try:
                ...
                if self._blob_path_for_revision_is_missing(manifest):
                    logger.warn(
                        "Blob for digest %s does not exist. Deleting tag manifest: %s", manifest, other_tag)
    ...
    def delete_entire_repository(self, repo):
        ...
        for layer in links:
            if layer in all_links_but_current:
                logger.debug(
                    "Blob found in another repository. Not deleting: %s", layer)
            else:
    ...
    def delete_repository_tag(self, repo, tag):
        ...
        tag_dir = os.path.join(self.registry_data_dir,
                               "repositories", repo, "_manifests/tags", tag)
        ...
        for manifest in manifests_for_tag:
            logger.debug(
                "Looking up filesystem layers for manifest digest %s", manifest)

            if self._manifest_in_same_repo(repo, tag, manifest):
                logger.debug(
                    "Not deleting since we found another tag using manifest: %s", manifest)
                continue
            else:
                ...
                if manifest in all_links_not_in_current_repo:
                    logger.debug(
                        "Not deleting the blob data since we found another repo using manifest: %s", manifest)
        ...
        for layer in layers_uniq:
            if self._layer_in_same_repo(repo, tag, layer):
                logger.debug(
                    "Not deleting since we found another tag using digest: %s", layer)
                continue
    ...
    def delete_untagged(self, repo):
        ...
        dir_for_revisions = os.path.join(
            repo_dir, "_manifests/revisions/sha256")
        for rev in os.listdir(dir_for_revisions):
            if rev not in tagged_revisions:
                revisions_to_delete.append(
                    os.path.join(dir_for_revisions, rev))
...
def main():
    ...
    if 'REGISTRY_DATA_DIR' in os.environ:
        ...
        registry_data_dir = "/opt/data/registry/docker/registry/v2"
